...
const GIFEncoder = function () {
  ...
  const start = (exports.start = function start() {
    ...
    out = new ByteArray();
    try {
      out.writeUTFBytes("GIF89a"); // 写入GIF文件头
      firstFrame = true;
      started = true;
      closeStream = false;
      return true;
    } catch (e) {
      console.error("Error writing GIF header:", e);
      return false;
    }
  });
  ...
  const stream = (exports.stream = function stream() {
    if (out === null) {
      throw new Error("Please call start method before calling stream");
    }
    return out;
  });
...
// Make GIFEncoder available globally
